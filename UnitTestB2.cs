...
{
    public class UnitTestB2
    {
        private Exercise.ProgramB2 prog;
        public UnitTestB2()
        {
            prog = new ProgramB2();
        }
        ...
        [InlineData("file/file1.txt", -3)]
        [InlineData("files/files1.txt", -2)]
        public void Test1(string values, int result)
        {
            var outcome = prog.CheckFileAvailability(values);
            
            Assert.True((int)outcome == result, $"For CheckFileAvailability: When trying to open a file, you should have returned state: {result} but did return {outcome}.");
        }
        ...
        [InlineData("file/file1.txt", -3, 0)]
        [InlineData("files/file01.txt", -2, 0)]
        [InlineData("files/files1.map", -2, 1)]
        ...
        public void Test3(string values, int result, int retries)
        {
            ...
            Assert.True((int)outcome == result, $"For PersistentFileOpen: When trying to open a file  in persistent mode which requires an object variable: You should have returned state: {result} but did return {outcome}.");
            ...
            if (retries > 0)
            {
                if (result == 0)
                {
                    ...
                    Assert.True((int)outcome == -2, "For PersistentFileOpen: When trying to open a file  in persistent mode: Opening a file the 2nd time but did not get correct response!");

                }
                else
                {
                    outcome = prog.PersistentFileOpen(values);
                    Assert.True((int)outcome == result, $"For PersistentFileOpen: When trying to open a file  in persistent mode: You should have returned file state: {result} but did return {outcome}.");
                }
            }
            else
            {
                prog = new ProgramB2();
                outcome = prog.PersistentFileOpen(values);
                Assert.True((int)outcome == result, $"For PersistentFileOpen: When trying to open a file  in persistent mode: Opening the file a 2nd time after closing the object once. You should have returned file state:{result} but did return {outcome}.");
        ...
        [InlineData("files/file1.txt", 0)]
        [InlineData("file/file1.txt", -3)]
        [InlineData("files/file01.txt", -2)]
        [InlineData("files/files1.map", -2)]
        ...
        public void Test4(string values, int result)
        {
            ...
            Assert.True((int)outcome == -4, $"For PersistentFileClose: When trying to close a file  in persistent mode: Trying to close a file which was not opened before should return CouldNotClose instead you returned: {outcome}");
            ...
            outcome = prog.PersistentFileOpen(values);
            if ((int)outcome == result)//file could be opened or not
            {
                if (result == 0)// file should now be open
                {
                    ...
                    Assert.True((int)outcome == -2, $"We have an open file but tried closing some other random file which did not work, state: {outcome} but should be showing a FileIssue");
                    ...
                    Assert.True((int)outcome == 0, $"We have an open file but closing it did not work, state: {outcome}");
                    ...
                    outcome = prog.PersistentFileClose(values);
                    Assert.True((int)outcome == -4, "We tried closing an open file twice but the system does not return the correct state: CouldNotClose");
                }
            }
        ...
        [InlineData("files/file1 aaa.txt", -2, 0, "")]
        ...
        public void Test5(string values, int result, int lines, string line)
        {
            ...
            Assert.True((int)openCheck == result, $"For PersistentFileRead: When trying to read a file  in persistent mode: Trying to open a file for persistent access did not work.");
            ...
            Assert.True((int)openCheck == result, $"For PersistentFileRead: Trying to close a file for persistent access did not work.");
            if (result != 0)
                return;
            ...
            outcome = prog.PersistentFileRead(lines);
            Assert.True(!outcome[lines - 1].Trim().Equals(line), $"For PersistentFileRead: Reading a second time should not return the same result.\nFrom the file, you should have returned {lines} lines where line nr.{lines - 1} is:\n {line} \nbut did return:\n {outcome[lines - 1]}.");
